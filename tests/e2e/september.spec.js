...
test("query personalization stays in the greeting while the approved final letter remains verbatim", async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem("september:nfc-progress:v1", JSON.stringify({
      v: 1,
  ...
  const finalLetter = "Anh không ở cạnh lúc em mở thiếp, nên gửi một xưởng nhỏ thay anh chuẩn bị mọi thứ. Bánh để em có một chút ngọt, hoa để ngày của em đẹp hơn. Còn anh chỉ muốn em biết: dù không ở đây, anh vẫn muốn có mặt trong ngày của em theo một cách nhỏ thôi.";
  await expect(page.getByText(finalLetter)).toBeVisible();
  ...
  await expect(directPage.getByText(finalLetter)).toBeVisible();
