...
if (APP_CONFIG.soundtrack.lyricsSrc !== "./public/audio/co-em-lyrics.txt") {
...
for (const relativePath of productionTextFiles) {
  ...
  if (
    /(?:href|src)=["']\/(?!\/)|url\(\s*["']?\/(?!\/)|["']\/(?:audio|fonts|images|src)\//iu.test(
      source,
    )
  ) {
    fail(`${relativePath} contains a root-absolute runtime path.`);
  }
}
